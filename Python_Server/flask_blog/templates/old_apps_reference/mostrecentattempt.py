...
from sqlalchemy import Column, ForeignKey, Integer, Text, UnicodeText, create_engine, MetaData, Table, insert
...
#db = Table('database.db',metadata, autoLoad=True)
...
## need to make using sqlalchemy language instead, raw queries don't work
...
def create_app():
    ...
    @auth_required()
    def index():
        # need to pull posts column using SQL alchemy
        #posts = db.select()
    ...
    # need to update to SQL alchemy language
